...
object ThumbnailManager {
    ...
    fun generateThumbnail(context: Context, sourceUri: Uri, isVideo: Boolean = false): String? {
        return try {
            ...
            val bitmap: Bitmap? = if (isVideo) {
                generateVideoThumbnail(context, sourceUri)
            } else {
    ...
    fun getCachedVideoThumbnail(context: android.content.Context, videoPath: String): String? {
        return try {
            val hash = videoPath.hashCode().toUInt().toString()
            val thumbDir = File(context.filesDir, "thumbnails")
            val thumbFile = File(thumbDir, "vthumb_$hash.jpg")
            if (thumbFile.exists()) thumbFile.absolutePath else null
        } catch (_: Exception) {
            null
        }
    }

    fun cacheVideoThumbnail(context: android.content.Context, videoPath: String, bitmap: Bitmap): String? {
        return try {
            val thumbDir = File(context.filesDir, "thumbnails")
            if (!thumbDir.exists()) thumbDir.mkdirs()
            val hash = videoPath.hashCode().toUInt().toString()
