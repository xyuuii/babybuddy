...
object CloudManager {
    ...
    suspend fun deleteFile(remotePath: String, config: CloudStorageConfig): Boolean = withContext(Dispatchers.IO) {
        try {
            when (config.protocol) {
                StorageProtocol.WEBDAV -> WebDavManager.deleteFile(config.toWebDavConfig(), remotePath).getOrDefault(false)
    ...
    suspend fun testConnection(config: CloudStorageConfig): Result<Boolean> = withContext(Dispatchers.IO) {
        fun verified(result: Result<Boolean>, protocolName: String): Result<Boolean> {
            return result.fold(
                onSuccess = { ok ->
                    if (ok) Result.success(true)
                    else Result.failure(Exception("$protocolName connection test did not pass"))
                },
            ...
            StorageProtocol.WEBDAV -> {
                ...
                result.fold(
                    onSuccess = { testResult ->
                        if (testResult.success) Result.success(true)
                        else Result.failure(Exception(testResult.message.ifBlank { "WebDAV connection test did not pass" }))
                    },
                    onFailure = { Result.failure(it) }
                )
            }
            StorageProtocol.SMB -> verified(SmbManager.testConnection(config), "SMB")
            StorageProtocol.FTP -> verified(FtpManager.testConnection(config), "FTP")
        }
    }
}
